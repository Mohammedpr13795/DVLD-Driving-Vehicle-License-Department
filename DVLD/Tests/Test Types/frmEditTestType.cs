...
namespace DVLD.Tests
{
    ...
    public partial class frmEditTestType: Form
    {
        ...
        private async void frmEditTestType_Load(object sender, EventArgs e)
        {
            ...
            _TestType = await clsTestType.Find( _TestTypeID);
        ...
        {
            return txtTitle.Text == _TestType.Title && _TestType.Description == txtDescription.Text && txtFees.Text == Math.Round(_TestType.Fees).ToString();
        }
        ...
        {
            // السماح بالأرقام وأزرار التحكم
            if (char.IsDigit(e.KeyChar) || char.IsControl(e.KeyChar)) return;

            // السماح بنقطة أو فاصلة كفاصل عشري (واحدة فقط)
            if ((e.KeyChar == '.' || e.KeyChar == ',') &&
               txtFees.Text.IndexOf('.') == -1 &&
               txtFees.Text.IndexOf(',') == -1)
            {
                return;
            }
