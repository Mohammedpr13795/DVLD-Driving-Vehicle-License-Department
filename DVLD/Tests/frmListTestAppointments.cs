...
namespace DVLD.Tests
{
    public partial class frmListTestAppointments: Form
    {
        ...
        private async void frmListTestAppointments_Load(object sender, EventArgs e)
        {
            ...
            _dtLicenseTestAppointments = await clsTestAppointment.GetApplicationTestAppointmentsPerTestTypeAsync(_LocalDrivingLicenseApplicationID,_TestType);
        ...
        private async void btnAddNewAppointment_Click(object sender, EventArgs e)
        {
            ...
            clsLocalDrivingLicenseApplication localDrivingLicenseApplication = await clsLocalDrivingLicenseApplication.FindByLocalDrivingAppLicenseIDAsync(_LocalDrivingLicenseApplicationID);

            if (await localDrivingLicenseApplication.DoesPassTestTypeAsync(_TestType))
            {
                MessageBox.Show("Person Already have passed this test , you cannot add new appointment", "Not allowed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (await localDrivingLicenseApplication.IsThereAnActiveScheduledTestAsync(_TestType))
            {
            ...
            //---
             clsTest LastTest = await localDrivingLicenseApplication.GetLastTestPerTestTypeAsync(_TestType);
